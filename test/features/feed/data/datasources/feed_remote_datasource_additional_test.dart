...
void main() {
  ...
  test('getFollowingFeed extracts track from playlist when track is null', () async {
    const token = 't';
    final playlistTrack = {
      'id': 'pl-track-1',
    ...
    final item = {
      ...
      'track': null,
      'playlist': {'id': 'pl-1', 'title': 'Pl', 'tracks': [playlistTrack]},
    };

    when(() => mockApiClient.getToken()).thenAnswer((_) async => token);
    when(() => mockDio.get('/feed', options: any(named: 'options'))).thenAnswer(
      (_) async => Response(requestOptions: RequestOptions(path: ''), data: {'data': [item]}, statusCode: 200),
    );
  ...
  test('getDiscoverFeed maps owner when artist missing username', () async {
    ...
    final discoverItem = {
      'id': 'd-1',
      'reason': {'label': 'L'},
      'track': {'id': 't-1', 'title': 'T', 'artist': {'id': 'a1'}},
    };

    when(() => mockApiClient.getToken()).thenAnswer((_) async => token);
    when(() => mockDio.get('/feed/discovery', options: any(named: 'options'))).thenAnswer(
      (_) async => Response(requestOptions: RequestOptions(path: ''), data: {'data': [discoverItem]}, statusCode: 200),
    );
