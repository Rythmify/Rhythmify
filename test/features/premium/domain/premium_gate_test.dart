...
void main() {
  group('PremiumGate', () {
    testWidgets('handleError returns true and shows bottom sheet for 403 SUBSCRIPTION_LIMIT_REACHED', (tester) async {
    ...
    testWidgets('handleError returns true if error string contains code', (tester) async {
      bool? result;
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: Builder(
              builder: (context) {
                return ElevatedButton(
                  onPressed: () {
                    result = PremiumGate.handleError(context, 'Error: SUBSCRIPTION_LIMIT_REACHED');
                  },
                  child: const Text('Tap'),
                );
              },
