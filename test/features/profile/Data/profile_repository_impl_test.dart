...
void main() {
  ...
  test('returns Right for successful operations', () async {
    when(() => datasource.getProfile(userId: 'u1')).thenAnswer((_) async => profile);
    ...
    when(() => datasource.uploadAvatar(filePath: 'a.png'))
        .thenAnswer((_) async => profile);
    when(() => datasource.deleteAvatar()).thenAnswer((_) async {});
    when(() => datasource.uploadCoverPhoto(filePath: 'c.png'))
        .thenAnswer((_) async => profile);
    ...
    when(() => datasource.getLikedTracks(userId: 'u1', page: 1, limit: 20))
    ...
    expect((await repository.uploadCoverPhoto(filePath: 'c.png')).isRight(), true);
    ...
    expect((await repository.getLikedTracks(userId: 'u1', page: 1, limit: 20)).isRight(), true);
  });

  test('maps known profile errors', () async {
    when(() => datasource.getProfile(userId: 'u1'))
    ...
    when(() => datasource.deleteCoverPhoto())
        .thenThrow(Exception('VALIDATION_FAILED'));
    when(() => datasource.followUser(userId: 'u2'))
        .thenThrow(Exception('RATE_LIMIT_EXCEEDED'));
    when(() => datasource.unfollowUser(userId: 'u2'))
        .thenThrow(Exception('socket timeout'));

    expect((await repository.getProfile(userId: 'u1')).fold((l) => l, (_) => null), isA<ServerFailure>());
    expect((await repository.updateProfile(displayName: 'N', city: 'C', country: 'EG', bio: 'B')).fold((l) => l, (_) => null), isA<ServerFailure>());
    expect((await repository.uploadAvatar(filePath: 'a.png')).fold((l) => l, (_) => null), isA<ServerFailure>());
    expect((await repository.deleteAvatar()).fold((l) => l, (_) => null), isA<ServerFailure>());
    expect((await repository.uploadCoverPhoto(filePath: 'c.png')).fold((l) => l, (_) => null), isA<ServerFailure>());
    expect((await repository.deleteCoverPhoto()).fold((l) => l, (_) => null), isA<ServerFailure>());
    expect((await repository.followUser(userId: 'u2')).fold((l) => l, (_) => null), isA<TooManyRequestsFailure>());
    expect((await repository.unfollowUser(userId: 'u2')).fold((l) => l, (_) => null), isA<NetworkFailure>());
  });
}
