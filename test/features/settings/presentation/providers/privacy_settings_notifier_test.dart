...
void main() {
  ...
  group('PrivacySettingsNotifier', () {
    ...
    group('save', () {
      test(
        'optimistically applies updated entity and keeps it after confirmed patch',
        () async {
          when(
            () => mockRepo.getPrivacySettings(),
          ).thenAnswer((_) async => tPrivacyEntity);

          final updated = tPrivacyEntity.copyWith(
          ...
          when(
            () => mockRepo.updatePrivacySettings(any(), any()),
      ...
      test(
        'keeps optimistic state when update throws (silent catch)',
        () async {
          when(
            () => mockRepo.getPrivacySettings(),
          ).thenAnswer((_) async => tPrivacyEntity);
      ...
      test(
        'passes previous state as "old" and updated entity to repository',
        () async {
          when(
            () => mockRepo.getPrivacySettings(),
          ).thenAnswer((_) async => tPrivacyEntity);
      ...
      test(
        'uses updated entity as "old" when state has no data before save',
        () async {
          ...
          final secondUpdate = firstUpdate.copyWith(
