...
class TrackDto {
  static Track fromJson(Map<String, dynamic> json) {
    ...
    return Track(
      ...
      artist:      data['artist'] as String? ?? data['user']?['display_name'] as String? ?? '',
      description: data['description'] as String?,
      coverImage:  data['cover_image'] as String? ?? data['artwork_url'] as String?,
      audioUrl:    data['audio_url'] as String? ?? data['stream_url'] as String? ?? '',
      streamUrl:   data['stream_url'] as String?,
      waveformUrl: data['waveform_url'] as String?,
      duration: Duration(seconds: data['duration'] as int? ?? data['duration_seconds'] as int? ?? 0),
      ...
      waveformData: data['waveform_data'] != null 
          ? List<double>.from(data['waveform_data'].map((x) => (x as num).toDouble())) 
          : null,
