...
class SearchRemoteSourceImpl implements SearchRemoteSource {
  ...
  Future<SearchResults> getSearchResultsTyped(String query, String type) async {
    ...
    final playlists = type == 'playlists'
        ? (data['playlists'] as List? ?? []).map((p) {
    ...
    final albums = type == 'albums'
        ? (data['albums'] as List? ?? []).map((a) {
